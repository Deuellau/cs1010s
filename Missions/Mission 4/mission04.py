...
def get_direction(line, train_position):
    ...
    if get_station_position(line, from_station_code) < get_station_position(line, to_station_code):
        return 0
    else:
        return 1
...
def get_schedules_at_time(train_schedule, time):
    res = ()
    for train_event in train_schedule:
        if get_schedule_time(train_event) == time:
            res += (train_event,)
    return res
...
def get_schedules_near_loc_id_in_line(train_schedule, line, loc_id):
    res = ()
    for train_event in train_schedule:
        train_position = get_location_id_in_line(train_event, line)
        if (abs(train_position - loc_id) <= 0.5):
            res += (train_event,)
    return res
...
def get_rogue_schedules_in_line(train_schedule, line, time, loc_id):
    filtered_schedule = get_schedules_at_time(train_schedule, time)
    filtered_schedule2 = get_schedules_near_loc_id_in_line(filtered_schedule, line, loc_id)
    return filtered_schedule2
...
def calculate_blame_in_line(full_schedule, valid_bd_events, line, scorer):
    for bd_event in valid_bd_events:
        ...
        filtered_events = get_rogue_schedules_in_line(full_schedule, line, time, loc_id)
        
        seen = ()
        for event in filtered_events:
            if get_train(event) not in seen:
                blame_train(scorer, get_train_code(get_train(event)))
                seen += (get_train(event),)
...
def find_rogue_train(scorer, max_score):
    rogue_train = tuple(filter(lambda x: x[1] == max_score, get_blame_scores(scorer)))
    return rogue_train[0][0]
